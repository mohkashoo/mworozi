...
if st.session_state.get("analysis_done"):
    ...
    # ── Share Report (brief, all languages, with recovery plan) ──
    ...
    if disease_key in TREATMENT_PLANS:
        ...
        for stage, task in TREATMENT_PLANS[disease_key]:
            steps_txt += f"  [{stage}] {task}\n"
    
    brief = f"Crop: {res['crop']}\nDisease: {res['disease']}\nSeverity: {res['severity']}\n\nTreatment:{res.get('treatment','')[:400].replace('##','').replace('*','')}{steps_txt}"
    
    if share_lang != "English" and gemini_client:
        with st.spinner(f"Translating to {share_lang}..."):
            translated = ask_gemini(f"Translate this crop diagnosis and recovery checklist into {share_lang}. Keep the structure, keep [Day 1] [Day 3] etc labels as-is. Only translate the words:\n\n{brief}", 800)
            if translated: brief = translated
    
    import urllib.parse, base64
    wa_url = f"https://wa.me/?text={urllib.parse.quote(brief[:1500])}"
    sms_url = f"sms:?body={urllib.parse.quote(brief[:400])}"
    
    col_wa, col_sms, col_pdf = st.columns([1, 1, 1])
    with col_wa:
    ...
    with col_pdf:
        brief_b64 = base64.b64encode(brief.encode()).decode()
        print_html = f"""<html><head><meta charset='utf-8'><style>body{{font-family:Arial;padding:20px;white-space:pre-wrap;font-size:14px;color:#333}}h1{{color:#059669;font-size:20px}}</style></head><body><h1>Mworozi Diagnosis Report</h1>{brief}<br><p><em>Generated by Mworozi AI — Frontiers GenAI Hackathon 2026</em></p></body></html>"""
        print_b64 = base64.b64encode(print_html.encode()).decode()
        st.markdown(f'<a href="data:text/html;base64,{print_b64}" download="mworozi_report.html"><button style="width:100%;padding:0.5rem;border-radius:8px;border:1px solid #8b5cf6;background:transparent;color:#8b5cf6;cursor:pointer;font-size:0.85rem">📄 Save PDF</button></a>', unsafe_allow_html=True)
        st.caption("Opens in browser → Print → Save as PDF")
